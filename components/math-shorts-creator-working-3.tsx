...
export default function MathShortsCreator() {
  const [imageList, setImageList] = useState<string[]>([]) // Changed: Array of images
  const [currentImageIndex, setCurrentImageIndex] = useState(0) // Changed: Track current image
  ...
  const handleNextStep = useCallback(async () => {
    if (currentStep < revealSteps.length) {
      ...
      try {
        ...
        setCurrentStep(1) // Start revealing the new image
      } catch (error) {
  ...
  const handlePrevStep = useCallback(async () => {
    if (currentStep > 1) {
      ...
      try {
        ...
        // Go to the last step of the previous image
        const steps = await detectTextLinesForReveal(await loadImage(imageList[prevIndex]))
        setCurrentStep(steps.length) // Show fully revealed previous image
      } catch (error) {
          ...
          {cameraStream && (
            <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800 text-center">
                ...
                Use Previous/Next buttons to control the reveal. Use screen recording on your device.
              </p>
            </div>
          )}
        </div>
