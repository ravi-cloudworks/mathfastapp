...
export default function MathShortsCreator() {
  ...
  const handleImageUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = Array.from(files).filter(file => {
      ...
      if (file.size > 10 * 1024 * 1024) {
        alert(`${file.name} is too large. Please upload images smaller than 10MB`)
        return false
      }
      return true
    })
  ...
  // Use useEffect to start camera when state changes to 'requesting'
  useEffect(() => {
  ...
  if (cameraState === 'denied') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
        <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-6 text-center">
          <h2 className="text-xl font-bold mb-4 text-gray-900">Camera Access Denied</h2>
      ...
      {currentImageSrc && (
        <div className="w-full max-w-2xl bg-white rounded-lg shadow-lg p-6 relative overflow-hidden">
          ...
          {cameraState === 'audio-only' && (
            <div className="absolute top-4 right-4 w-16 h-16 bg-green-600 rounded-full flex items-center justify-center border-2 border-white shadow-lg z-30">
          ...
          {(cameraState === 'ready' || cameraState === 'audio-only') && currentStep === 0 && (
            <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800 text-center">
                ...
                {cameraState === 'ready' ? 'Camera is ready.' : 'Voice-only mode active.'} Click "Start Revealing" to begin.
              </p>
            </div>
          )}
          ...
          {currentStep > 0 && (cameraState === 'ready' || cameraState === 'audio-only') && (
            <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800 text-center">
                ...
                Step {currentStep} of {revealSteps.length} - Use Previous/Next to control reveal
              </p>
            </div>
          )}
        </div>
